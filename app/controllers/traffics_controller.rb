class TrafficsController < ApplicationController
  def index
    ...
    @stores = @q.result(distinct: true).order(created_at: :desc).paginate(page: params[:page], per_page: 6)
  ...
  def show
    ...
    @hotspots = @store.spots.select {|spot| spot.tags.include?('hotspot') }
    @shoppings = @store.spots.select {|spot| spot.tags.include?('shopping') }
  end
end
